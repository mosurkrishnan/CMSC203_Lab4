...
public class TwoDimArrayPractice extends JFrame
{
    ...
    // Write your code here:
	   for (int row = 0; row < intArray.length; row++) {
		   for (int column = 0; column < intArray[row].length; column++) {
			   ...
			   // 	use this in last row - animate( row, column );
			   animate( row, column );		   
		   }	   
	   }
  }
   ...
   public void setValues( int value, int row )
   {
    ...
    // Write your code here:
	   for (int column = 0; column < intArray[row].length; column++) {
		   ...
		   // assumption the loop goes from column-0 to column-1 to 2 within the specified row (passed parameter)
		   animate( row, column ); // provided
	   }
   }
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   int minimum = 10_000_000; // set for some arbitrary high number
	   for (int row = 0; row < intArray.length; row++) {
		   if(intArray[row][column] < minimum) {
			   minimum = intArray[row][column];
		   }
		   animate( row, column, minimum ); // CHECK - IF THIS SHOULD BE OUTSIDE/INSIDE for LOOP
	   }
   ...
   */
   public int countFound( int value )
   {
    ...
    // Write your code here:
	   int num = 0; // initialize to zero
	   for (int row = 0; row < intArray.length; row++) {
		   for (int column = 0; column < intArray[row].length; column++) {
			   if(intArray[row][column] == value) {
				   num++;
			   }	
			   animate( row, column, num ); // should be VALUE and not NUM
		   }		   
	   }
